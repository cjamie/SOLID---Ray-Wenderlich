...
class MultipleResponsibilityViewController: UIViewController {
    ...
    private lazy var navigationControllerDelegate = NavigationControllerDelegate(sender: self) { ShowAnimationTransition(self.selectedIndex) }
    ...
    override func viewDidLoad() {
        ...
        navigationControllerDelegate.start()
    }
...
final class NavigationControllerDelegate: NSObject, UINavigationControllerDelegate {
    ...
    private let senderVC: UIViewController
    private let transitioningCreator: (() -> UIViewControllerAnimatedTransitioning)
    
    init(sender: UIViewController, transitioningCreator: @escaping (() -> UIViewControllerAnimatedTransitioning)) {
        self.senderVC = sender
        self.transitioningCreator = transitioningCreator
        super.init()
    }
    
    func start() {
        senderVC.navigationController?.delegate = self
    }
    ...
    func navigationController(
        ...
        if fromVC == senderVC, toVC is AnimalDetailsViewController {
            return transitioningCreator()
        }
        return nil
    }
}

final class ShowAnimationTransition: NSObject, UIViewControllerAnimatedTransitioning {
    
    private let selectedIndex: IndexPath

    init(_ selectedIndex: IndexPath) {
